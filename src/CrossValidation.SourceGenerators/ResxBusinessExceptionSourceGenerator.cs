...
public class ResxBusinessExceptionSourceGenerator : IIncrementalGenerator
{
        ...
        var result =
            $$"""
              ...
              {{GenerateParentClassStart(parentClass, ref numberOfParentClasses)}}
                  ...
                  {
                      {{GenerateProperties(constructorParameters)}}
    ...
    private static string GenerateProperties(ParameterSyntax[]? constructorParameters)
    {
        ...
        foreach (var parameter in constructorParameters)
        {
            ...
            properties.AppendLine($"public {parameter.Type!.ToString()} {char.ToUpper(identifier[0])}{identifier.Substring(1)} => {identifier};");
        }
